...
rl.on('line', function(line){
   ...
   while (idx < lineRecords.length) {
    ...
    if(isHeader == false){
        ...
        if((tempData.IndicatorCode == "SP.DYN.CBRT.IN")&& tempData.CountryName =="India"){
        ...
        if((tempData.IndicatorCode == "SP.DYN.CDRT.IN")&& tempData.CountryName =="India"){
                    arr[parseInt(tempData.Year)-1960][0]=parseInt(tempData.Year);
                    var count = parseFloat(tempData.Value);
...
rl.on('close', function(){
    ...
    var obj1 = {};
    obj1.Year = arr[i][0];
    obj1["Birth rate, crude (per 1,000 people)"]=parseFloat(arr[i][1]);
    obj1["Death rate, crude (per 1,000 people)"]=parseFloat(arr[i][2]);

  //console.log(obj1.toString());
    }
    jsonData.push(obj1);
    fs.writeFileSync('jsonData/data2.json',JSON.stringify(jsonData), encoding = "utf8");
}
